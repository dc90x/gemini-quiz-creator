...
class QuizGenerator:
    ...
    def generate_quiz(self) -> list:
        ...
        # Makes sure that the right number of questions are created after errors and duplications
        while len(self.question_bank) < self.num_questions:
            question_str = self.generate_question_with_vectorstore()

            try:
                question_dict = json.loads(question_str) # Convert the JSON String to a dictionary
            ...
            if self.validate_question(question_dict):
                ...
                self.question_bank.append(question_dict) # Add the valid and unique question to the bank
            else:
    ...
    def validate_question(self, question: dict) -> bool:
        ...
        for i in range(len(self.question_bank) - 1):
            if question_text == self.question_bank[i].get("question"):
                is_unique = False
